/* This is the Poseidon hash function configurations.

- S-Box Alpha (17): The lowest valid exponent satisfying gcd(alpha, p-1) = 1 ensuring invertability.

- State Size (t = 3):  a 2-to-1 compression ratio (2 rate, 1 capacity) which is standard in protocols
like zCash and TornadoCash

- Round Structure (37 Full, 8 Partial)

- The ARK constants are derived from public sources to ensure Nothing Up My Sleeve is adhered to. 
From official Poseidon cryptographic implementation scripts.

Internal State (Size t = 3, 2r + 1c) c is initialized to 0.
      [ State 0 ]   [ State 1 ]   [ State 2 (c) ]
           │             │             │
           ▼             ▼             ▼
    ┌─────────────┐┌─────────────┐┌─────────────┐
    │ + ARK[r][0] ││ + ARK[r][1] ││ + ARK[r][2] │  ◄── Additive Round Keys (Confusion)
    └──────┬──────┘└──────┬──────┘└──────┬──────┘
           │             │             │
           ▼             ▼             ▼
    ┌─────────────┐┌─────────────┐┌─────────────┐
    │   x^17      ││   x^17      ││   x^17      │  ◄── S-Box Layer (Full vs Partial rounds)
    └──────┬──────┘└──────┬──────┘└──────┬──────┘
           │             │             │
           └─────────────┼─────────────┘
                         ▼
             ┌───────────────────────┐
             │   3x3 MDS Matrix      │             ◄── Diffusion Matrix Layer
             └───────────┬───────────┘
                         │
      ┌──────────────────┼──────────────────┐
      ▼                  ▼                  ▼
[ New State 0 ]    [ New State 1 ]    [ New State 2 ] ──► To Next Round
...
pub fn poseidon_test_params<F: PrimeField>() -> PoseidonConfig<F> {
    // Exponent for the non-linear S-box layer (x^alpha)
    let alpha = 17;

    // 3x3 MDS diffusion matrix, invertible and provides strong avalache effects accross state
    let mds = vec![
        vec![
            F::from_str(
